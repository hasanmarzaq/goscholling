...
import (
	...
	// "goschollingnew/api/security"
	// "goscholling/api/security"
	"html"
	"log"
	"os"
	"strings"
	...
	"github.com/hasanmarzaq87/goscholling/api/security"
	"gorm.io/gorm"
...
type User struct {
	...
	Religion   string `gorm:"size:255;null;" json:"religion"`
	AvatarPath string `gorm:"size:255;null;" json:"avatar_path"`
	Signature  string `gorm:"size:255;null;" json:"signature"`
	StatusID   uint32 `gorm:"not null" json:"status_id"`
	RoleID     uint32 `gorm:"not null" json:"role_id"`
	Role       Role
	IsActive   uint32 `gorm:"not null" json:"is_active"`
...
func (u *User) FindAllUsers(db *sql.DB) (*[]User, error) {
	var err error
	var users []User
	rows, err := db.Query("SELECT s.id,s.username,s.email,r.name FROM users s LEFT JOIN roles r on s.role_id = r.id")
	fmt.Println(rows)
	// err = db.Debug().Model(&Product{}).Limit(20).Find(&users).Error
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&user.ID, &user.Username, &user.Email, &role.Name); err != nil {
			log.Fatal(err)
		}
		user.Role = role
